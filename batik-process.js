...
import {getCollectedMemories} from './journey-state.js';
const panel=document.createElement('dialog');panel.className='reveal-workshop';
panel.innerHTML='<div class="process-mode-choice"><h2>你想怎样完成这块布？</h2><button id="process-mode-manual">亲手完成</button><button id="process-mode-auto">自动演示</button></div><div class="batik-stage"><canvas aria-label="上蜡白布的浸染与热水脱蜡过程"></canvas><img hidden src="./老师指定蜡染-无圆形.png" alt="老师指定的蓝白蜡染图" /></div><div class="batik-controls"><span aria-live="polite"></span><button id="process-next">将白布浸入染液</button><button id="switch-process-mode" hidden>切换为手动</button><button id="batik-zoom" hidden>放大细看</button><button id="leave-reveal">回到亭子 ×</button></div>';document.body.append(panel);
...
let stage=0,frame=0,source=null,busy=false,generation=0,processMode=null,autoTimer=0;
const layers=[];
...
function showEnding(){startEpilogue(getCollectedMemories().filter(k=>memories[k]).slice(-2).map(k=>memories[k]).join('')||'你从外婆的木桌出发，把这块白布带回了蓝里。');}
let gesture=null,manual=0;
canvas.style.touchAction='none';
canvas.addEventListener('pointerdown',e=>{if(processMode!=='manual'||busy||!source||stage===4)return;if(stage===2){stage++;manual=0;draw(stage,1);sync();return;}gesture={id:e.pointerId,x:e.clientX,y:e.clientY,base:manual};canvas.setPointerCapture(e.pointerId);dispatchEvent(new CustomEvent('batik-process',{detail:stage+1}));});
...
function prepare(){const crop={x:.148,y:.153,w:.648,h:.733};canvas.width=1200;canvas.height=Math.round(1200*image.naturalHeight*crop.h/(image.naturalWidth*crop.w));ctx.drawImage(image,image.naturalWidth*crop.x,image.naturalHeight*crop.y,image.naturalWidth*crop.w,image.naturalHeight*crop.h,0,0,canvas.width,canvas.height);source=ctx.getImageData(0,0,canvas.width,canvas.height);
 ...
 }draw(stage,1);next.disabled=false;scheduleNextAutoStep();
}
...
function sync(){const choosing=!processMode;panel.querySelector('.process-mode-choice').hidden=!choosing;caption.textContent=choosing?'选择亲手操作，或观看自动演示。':stages[stage][0];next.textContent=stages[stage][1];next.hidden=choosing||processMode!=='auto'||stage===4;panel.querySelector('#switch-process-mode').hidden=processMode!=='auto'||stage===4;panel.querySelector('#batik-zoom').hidden=stage!==4;canvas.hidden=false;image.hidden=true;gestureHint.hidden=choosing||processMode!=='manual'||stage===4;gestureHint.textContent=['按住布面，向下拖动，让白布入染','按住布面，向上提起，观察空气带出的蓝','轻触布面，放入热水；等待热量融去蜡','清洗后的布，向一侧拖开，展开全幅',''][stage];if(stage===4)showEnding();}
export function openWorkbench(){if(panel.open)return;stage=0;manual=0;gesture=null;processMode=null;ending.hidden=true;busy=false;next.disabled=false;panel.showModal();sync();if(source)draw(0,0);else if(image.complete&&image.naturalWidth)prepare();dispatchEvent(new CustomEvent('story-panel',{detail:true}));}
image.addEventListener('load',prepare);image.addEventListener('error',()=>{caption.textContent='布面未能载入，请返回亭子后重试。';next.disabled=true;});
function scheduleNextAutoStep(){clearTimeout(autoTimer);if(processMode==='auto'&&stage<4&&panel.open)autoTimer=setTimeout(()=>next.click(),600)}
next.onclick=()=>{if(busy||!source||stage>=4)return;busy=true;next.disabled=true;const target=stage+1,token=++generation;caption.textContent=['','染液浸入纤维，蜡覆盖的地方保留着白。','提出布面，接触空气，蓝色渐渐变深。','热水使蜡融化，蜡离开纤维，白色花鸟显露。','洗去残蜡与浮色，晾干，展开山野。'][target];next.textContent='请稍候…';dispatchEvent(new CustomEvent('batik-process',{detail:target}));const start=performance.now(),duration=matchMedia('(prefers-reduced-motion: reduce)').matches?100:[0,3000,3500,4000,1800][target];function tick(now){if(token!==generation||!panel.open)return;const t=Math.max(0,Math.min(1,(now-start)/duration));draw(target,t);if(t<1)frame=requestAnimationFrame(tick);else{stage=target;busy=false;next.disabled=false;sync();scheduleNextAutoStep();}}frame=requestAnimationFrame(tick);};
panel.querySelector('#process-mode-manual').onclick=()=>{processMode='manual';sync()};
panel.querySelector('#process-mode-auto').onclick=()=>{processMode='auto';sync();scheduleNextAutoStep()};
panel.querySelector('#switch-process-mode').onclick=()=>{generation++;cancelAnimationFrame(frame);clearTimeout(autoTimer);busy=false;next.disabled=false;processMode='manual';sync()};
...
panel.addEventListener('close',()=>{generation++;cancelAnimationFrame(frame);clearTimeout(autoTimer);panel.classList.remove('is-zoomed');panel.querySelector('#batik-zoom').textContent='放大细看';dispatchEvent(new CustomEvent('story-panel',{detail:false}));dispatchEvent(new Event('pavilion-return'));});
addEventListener('journey-reset',()=>{generation++;cancelAnimationFrame(frame);clearTimeout(autoTimer);stage=0;manual=0;processMode=null;busy=false;if(panel.open)panel.close();});
