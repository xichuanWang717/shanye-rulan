...
test('the camera keeps the pavilion flock centered as it follows the open sky corridor', () => {
  ...
  assert.ok(risingFocus.x > 8.5 && risingFocus.x < 9.5);
  camera.updateMatrixWorld(true);
...
test('normal flight gives the completed cloth motif a visible hold before fading', () => {
  ...
  const gathering = transition.getFlightTransitionFrame(3.45, true, false);
  assert.equal(gathering.visible, true);
  assert.equal(gathering.phase, 'gather');
  assert.ok(Math.abs(gathering.gather - 0.5) < 1e-12);
  assert.equal(gathering.veil, 0);
  assert.deepEqual(transition.getFlightTransitionFrame(5.25, true, false), {
    visible: true,
    phase: 'cloth',
    gather: 1,
    veil: 0,
