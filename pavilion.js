...
import {getPavilionBirdPosition} from './pavilion-flight-path.js?v=3';
import {arePavilionBirdsVisible,isPavilionFlightComplete} from './flight-transition-state.js?v=2';
export function createPavilion(scene){
 ...
 function update(time,dt,reduced){
  ...
  flock.forEach((o,i)=>{const rise=Math.max(0,flight-(i%8)*.12),position=getPavilionBirdPosition(rise,i);o.b.position.set(position.x,position.y,position.z);o.b.visible=flight>=0&&!done&&arePavilionBirdsVisible(flight,reduced)&&flight>=(i%8)*.12;o.b.rotation.z=Math.sin(rise*.8+o.offset)*.18;o.wings.forEach(w=>w.rotation.z=w.userData.side*(.25+Math.sin(time*12+o.offset)*.65));});
 }
