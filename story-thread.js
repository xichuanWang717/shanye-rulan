...
const observed=new Set();addEventListener('observe-memory',e=>observed.add(e.detail));addEventListener('story-restart',()=>observed.clear());
...
let timeout;
addEventListener('pavilion-flight',()=>{clearTimeout(timeout);discovery.textContent=observed.has('stream')?'布上的曲线，原来是刚才那道溪水。外婆把山野，留在了这里。':observed.has('flower')?'布上的花，原来也有山野里舒展的模样。你开始懂得外婆的笔。':'晾晒布上的花、水和鸟，与山野彼此呼应。原来外婆画下的，是她认真看过的生活。';discovery.hidden=false;timeout=setTimeout(()=>discovery.hidden=true,3600);});
